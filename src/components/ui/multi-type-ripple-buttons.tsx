...
const RippleButton: React.FC<RippleButtonProps> = ({
  ...
  if (variant === 'hover') {
    ...
    return (
      <>
        <style suppressHydrationWarning dangerouslySetInnerHTML={{ __html: JS_RIPPLE_KEYFRAMES }} />
        <style suppressHydrationWarning dangerouslySetInnerHTML={{ __html: dynamicGridHoverStyles }} />
        <button suppressHydrationWarning className={hoverButtonFinalClassName} onClick={handleButtonClick} disabled={disabled}>
          <span className="relative z-[10] pointer-events-none">
            <HydrationSafe>{children}</HydrationSafe>
          </span>
          <div className="hover-variant-ripple-container absolute inset-0 z-[1] overflow-hidden pointer-events-none">
            <div className="hover-variant-grid-container absolute inset-0 z-[10] grid grid-cols-[repeat(36,1fr)] grid-rows-[repeat(12,1fr)] pointer-events-auto">
              {Array.from({ length: GRID_HOVER_TOTAL_CELLS }).map((_, i) => (
                <div key={i} className="hover-variant-grid-cell w-full h-full" />
              ))}
              <div className="hover-variant-visual-ripple absolute z-[1] w-0 h-0 -translate-x-1/2 -translate-y-1/2 rounded-full pointer-events-none" />
            </div>
            <div className="absolute inset-0 z-[0]" style={{ backgroundColor: hoverBaseColor }} />
          </div>
          {jsRippleElements}
        </button>
      </>
  ...
  if (variant === 'hoverborder') {
    ...
    return (
      <>
        <style suppressHydrationWarning dangerouslySetInnerHTML={{ __html: JS_RIPPLE_KEYFRAMES }} />
        <style suppressHydrationWarning dangerouslySetInnerHTML={{ __html: dynamicGridHoverStyles }} />
        <button suppressHydrationWarning className={hoverBorderButtonFinalClassName} onClick={handleButtonClick} disabled={disabled}>
          <span className="relative z-[10] pointer-events-none">
            <HydrationSafe>{children}</HydrationSafe>
          </span>
          <div className="hoverborder-variant-ripple-container absolute inset-0 z-[1] overflow-hidden pointer-events-none border border-transparent">
            <div className="hoverborder-variant-grid-container absolute inset-0 z-[10] grid grid-cols-[repeat(36,1fr)] grid-rows-[repeat(12,1fr)] pointer-events-auto">
              {Array.from({ length: GRID_HOVER_TOTAL_CELLS }).map((_, i) => (
                <div key={i} className="hoverborder-variant-grid-cell w-full h-full" />
              ))}
              <div className="hoverborder-variant-visual-ripple absolute z-[1] w-0 h-0 -translate-x-1/2 -translate-y-1/2 rounded-full pointer-events-none" />
            </div>
          </div>
          {jsRippleElements}
        </button>
  ...
  return (
    <>
      ...
      <button
        ...
        className={["relative overflow-hidden transition-colors duration-200", className].filter(Boolean).join(" ")}
        onClick={handleButtonClick}
        disabled={disabled}
      >
        <span className="relative z-10"><HydrationSafe>{children}</HydrationSafe></span>
        {jsRippleElements}
      </button>
    </>
  );
