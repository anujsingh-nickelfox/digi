...
const ShaderCanvas = () => {
  ...
  const [backgroundColor, setBackgroundColor] = useState([0.0, 0.0, 0.0]); // Default to black

  useEffect(() => {
    // We strictly use black/white, so background is black (0,0,0) in dark mode
    // or we can just keep it black as requested for "black & white only" theme
  ...
  useEffect(() => {
    ...
    const fragmentShaderSource = ` 
       ...
       void main(){ 
         ...
         // Black & White only: use grayscale for foreground
         float gray = (v.x + v.y + (.7-v.y*v.x)) / 3.0;
         vec3 foregroundColor = vec3(gray); 
  ...
  return <canvas ref={canvasRef} className="absolute top-0 left-0 w-full h-full block z-0 bg-black" />;
};
...
export const PricingCard = ({
  ...
  const cardClasses = ` 
     ...
     ${isPopular ? 'scale-105 relative ring-2 ring-white/20 shadow-2xl bg-white/10' : ''} 
   `;
  const buttonClasses = ` 
     mt-auto w-full py-2.5 rounded-xl font-semibold text-[14px] transition font-sans 
     ${buttonVariant === 'primary'
      ? 'bg-white hover:bg-off-white text-black'
  ...
  return (
    <div className={cardClasses.trim()}>
      {isPopular && (
        <div className="absolute -top-4 right-4 px-3 py-1 text-[12px] font-semibold rounded-full bg-white text-black">
          Most Popular
        </div>
      )}
      <div className="mb-3">
        ...
        <ul className="flex flex-col gap-2 text-[14px] text-white/90 mb-6 font-sans">
          {features.map((feature, index) => (
            <li key={index} className="flex items-center gap-2">
              <CheckIcon className="text-white w-4 h-4" /> {feature}
            </li>
          ))}
        </ul>
      </HydrationSafe>
...
export const ModernPricingPage = ({
  ...
  return (
    <div className="bg-black text-white min-h-screen w-full overflow-hidden relative">
      ...
      <main className="relative z-10 w-full min-h-screen flex flex-col items-center justify-center px-4 py-24 lg:py-48">
        <div className="w-full max-w-5xl mx-auto text-center mb-14">
          <h1 className="text-[48px] md:text-[64px] font-extralight leading-tight tracking-[-0.03em] bg-clip-text text-transparent bg-gradient-to-r from-white via-white/80 to-white/60 font-display">
            {title}
          </h1>
